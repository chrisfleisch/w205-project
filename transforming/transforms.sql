...
CREATE TABLE altredditarchive AS SELECT id, timestamp, whiskyname, reviewerusername, link, cast(Rating as int), style, cast(bottleprice as float), reviewdate, altbrand FROM altredditarchive_raw;

DROP TABLE altvaprices;
CREATE TABLE altvaprices AS SELECT id, description, code, brand, cast(Size as float), cast(Age as float), cast(Proof as float), cast(Price as float), altbrand FROM altvaprices_raw;
