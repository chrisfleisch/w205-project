...
CREATE TABLE reddit_bottles AS SELECT altbrand, sum(Rating)/count(Rating) as avg_rating, count(Rating) as num_reviews, sum(altbottleprice) as altbottleprice FROM redditarchive WHERE rating > 0 GROUP BY altbrand;
